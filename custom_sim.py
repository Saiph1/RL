...
class LunarLander(gym.Env, EzPickle):
    ...
    * The angular velocity is in units of 0.4 radians per second. In order to convert to radians per second, the value needs to be multiplied by a factor of 2.5.
    ...
    def __init__(
        ...
        # Higher value of gravity so that it is realistic
        # assert (
        ...
        if self.continuous:
            ...
            # testing: originally spaces.Discrete(4)
    ...
    def reset(
        ...
        *,
        ...
        smooth_y = [
            0.2 * (height[i - 1] + height[i + 0] + height[i + 1])
            for i in range(CHUNKS)
        ]

        # Adjust the height around the result position
        smooth_y[0] = self.helipad_y
        smooth_y[1] = self.helipad_y
        smooth_y[2] = self.helipad_y
        ...
        # This line set the new custom position of the starting point
        initial_y = smooth_y[len(smooth_y)-1]+2
        initial_x = 18
    ...
    def step(self, action):
        ...
        if (self.continuous and np.abs(action[1]) > 0.5) or (
            not self.continuous and action in [1, 2]
        ):
            ...
            if self.continuous:
                ...
                # Original: action = 1 is left, action = 3 is right
                # Now: action 1 is left, 2 is right, 0 is do nothing, here the main engin is not used.  
                # if action == 1: 
                #     direction = -1
                # elif action == 2:
                #     direction = 1
                direction = action - 2
                # Original: direction = action - 2
                s_power = 1.0
        ...
        state = [
            ...
            1.0 if self.legs[0].ground_contact else -1.0,
            1.0 if self.legs[1].ground_contact else -1.0,
        ]
        assert len(state) == 8
        ...
        pos_goal_x = (self.helipad_x2 - self.helipad_x1)/2
        # initial_y = smooth_y[len(smooth_y)-1]+2
        # initial_x = 18
        
        shaping = (
            - 10000 * np.sqrt((pos_goal_x-self.lander.position.x) * (pos_goal_x-self.lander.position.x) + (self.helipad_y-self.lander.position.y) * (self.helipad_y-self.lander.position.y))
            - 1000 * np.sqrt((vel.x)*(vel.x)+(vel.y)*(vel.y))
            # - 1000 * np.sqrt(state[2] * state[2] + state[3] * state[3])
            - 100 * abs(self.lander.position.y-self.helipad_y)
            - 20000 * abs(state[4])
            # + 10 * state[6]
            # + 10 * state[7]
        )  # And ten points for legs contact, the idea is if you
        ...
        if self.lander.position.x >= self.helipad_x1 and self.lander.position.x <= self.helipad_x2:
            reward += 100
            if self.lander.position.y<self.helipad_y+1:
                reward += 400
            if abs(state[4]) < 1:
                reward += 2000
            if np.sqrt((vel.x)*(vel.x)+(vel.y)*(vel.y)) < 0.2: 
                reward += 1000
            terminated = True
        ...
        #     reward -=1000
        #     terminated = True
        if self.game_over or abs(state[0]) >= 1.0 or abs(state[1]) >= 1.7:
            reward -=10000
            terminated = True
